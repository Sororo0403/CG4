...
bool ContainsAny(const std::wstring &text,
                 std::initializer_list<std::wstring_view> tokens) {
...
bool IsLikelyLinearDataTexture(const std::filesystem::path &path) {
    ...
    return ContainsAny(text,
                       {L"normal", L"_nor", L"-nor", L"nrm", L"roughness",
                        L"_rough", L"-rough", L"metallic", L"metalness",
                        L"_metal", L"-metal", L"ambientocclusion", L"occlusion",
                        L"_ao", L"-ao", L"_arm", L"-arm", L"_orm", L"-orm",
                        L"opacity", L"alpha", L"height", L"displacement",
                        L"mask", L"scattering"});
}
...
bool DecodeResolvedTextureFile(const std::filesystem::path &resolvedPath,
    ...
    const DirectX::WIC_FLAGS wicFlags =
        useSrgb ? DirectX::WIC_FLAGS_FORCE_SRGB
                : DirectX::WIC_FLAGS_IGNORE_SRGB;
    if (FAILED(DirectX::GetMetadataFromWICFile(
            resolvedPath.c_str(), wicFlags, metadata)) ||
        !TextureLimits::IsMetadataWithinBudget(metadata)) {
        return false;
    }
    return SUCCEEDED(DirectX::LoadFromWICFile(
        resolvedPath.c_str(), wicFlags, &metadata, scratch));
}
...
bool DecodeMemoryForLoad(const uint8_t *data, size_t size,
    ...
    if (FAILED(DirectX::GetMetadataFromWICMemory(
            data, size, wicFlags, metadata)) ||
        !TextureLimits::IsMetadataWithinBudget(metadata)) {
        return false;
    }
    if (FAILED(DirectX::LoadFromWICMemory(
            data, size, wicFlags, &metadata, scratch))) {
        return false;
    }
...
DecodeResolvedFileForAsync(const std::wstring &resolvedPathText) {
    ...
    decoded.succeeded =
        DecodeResolvedTextureFile(resolvedPath, decoded.scratch,
                                  decoded.metadata,
