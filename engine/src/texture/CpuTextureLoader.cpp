...
bool CopyRgbaImage(const DirectX::Image &source,
                   CpuTextureRgbaImage &destination) {
...
bool LoadRgba8FromFile(const std::wstring &filePath,
    ...
    if (!TextureManagerDecoding::DecodeFileForRawPixels(resolvedPath, source,
                                                        metadata)) {
        return false;
    }
    ...
    if (baseImage->format != DXGI_FORMAT_R8G8B8A8_UNORM) {
        if (FAILED(DirectX::Convert(*baseImage, DXGI_FORMAT_R8G8B8A8_UNORM,
            ...
            !TextureLimits::AreImagesWithinDecodedBudget(
                rgbaImage.GetImages(), rgbaImage.GetImageCount())) {
            return false;
        }
