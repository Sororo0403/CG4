...
void LightingScene::RefreshStats() {
    ...
    stats_.hasSun = lighting_.keyLightColor.w > 0.5f &&
                    (lighting_.keyLightColor.x > 0.0001f ||
                     lighting_.keyLightColor.y > 0.0001f ||
                     lighting_.keyLightColor.z > 0.0001f);
    for (const PointLight &pointLight : lighting_.pointLights) {
        if (pointLight.colorIntensity.w > 0.0001f &&
            pointLight.positionRange.w > 0.0001f) {
    ...
    if (spotLight.angleParams.w > 0.5f &&
        spotLight.colorIntensity.w > 0.0001f &&
