...
bool PostProcessSystem::IsReady() const {
    return dxCommon_ != nullptr && srvManager_ != nullptr &&
           ...
           state_->copyPipelineState && state_->bloomRootSignature &&
           state_->bloomExtractPipelineState &&
           state_->bloomDownsamplePipelineState &&
           state_->bloomUpsamplePipelineState && HasConstantBuffers() &&
           HasBloomResources();
}
...
void PostProcessSystem::Initialize(DirectXCommon *dxCommon,
    ...
    Resize(width, height);
    if (!state_->rootSignature || !state_->pipelineState ||
        !state_->copyPipelineState || !state_->bloomRootSignature ||
        !state_->bloomExtractPipelineState ||
        !state_->bloomDownsamplePipelineState ||
        !state_->bloomUpsamplePipelineState || !HasConstantBuffers() ||
...
bool PostProcessSystem::Finalize(bool allowFrameAbort) {
    const bool hasGpuResources =
        !state_->constantFrames.empty() || state_->pipelineState ||
...
bool PostProcessSystem::RequiresPostProcess() const {
    return state_->profile.colorGrade.mode != PostProcessColorMode::None ||
           ...
           HasRandomNoise(state_->profile) ||
           state_->profile.radialBlur.strength > 0.0f ||
           state_->profile.sceneDim.strength > 0.0f ||
           HasToon(state_->profile);
}
...
void PostProcessSystem::Draw(D3D12_GPU_DESCRIPTOR_HANDLE textureHandle,
    ...
    if (!dxCommon_ || !srvManager_ || !state_->rootSignature ||
        !state_->pipelineState || !state_->copyPipelineState ||
        !HasConstantBuffers()) {
        return;
    }
    ...
    const bool bloomRequested =
        constants.bloomEnabled != 0 && constants.bloomIntensity > 0.0f;
    const bool bloomBuilt =
        bloomRequested ? BuildBloom(textureHandle, constants) : false;
    if (!bloomBuilt) {
    ...
    *constantFrame->mapped = constants;
    ...
    PIXBeginEvent(commandList, 0,
                  requiresPostProcess ? "PostProcess.Full"
                                      : "PostProcess.Copy");
    ID3D12DescriptorHeap *heaps[] = {heap};
    commandList->SetDescriptorHeaps(1, heaps);

    const D3D12_GPU_DESCRIPTOR_HANDLE bloomHandle =
        bloomBuilt ? srvManager_->GetGpuHandle(state_->bloomSrvStart)
                   : textureHandle;
    ...
    commandList->SetPipelineState(
        requiresPostProcess ? state_->pipelineState.Get()
    ...
    commandList->SetGraphicsRootConstantBufferView(
        3, constantFrame->resource->GetGPUVirtualAddress());
