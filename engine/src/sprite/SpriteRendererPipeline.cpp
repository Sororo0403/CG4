...
bool SpriteRenderer::HasAllPipelineStates() const {
    return std::all_of(
        std::begin(state_->pipelineStates), std::end(state_->pipelineStates),
...
void SpriteRenderer::CreatePipelineState() {
    ...
    auto psModulate =
        ShaderCompiler::Compile(ShaderPaths::SpritePS, "mainModulate",
                                "ps_6_6");
    ...
    for (uint32_t target = 0;
        ...
        if (FAILED(dxCommon_->GetDevice()->CreateGraphicsPipelineState(
                &desc,
                IID_PPV_ARGS(
                    &state_->pipelineStates
                         [target]
                         [static_cast<uint32_t>(PipelineKind::Alpha)])))) {
            resetPipelines();
            return;
        }
        ...
        desc.BlendState = blend;
        desc.PS = {psModulate->GetBufferPointer(),
                   psModulate->GetBufferSize()};
        if (FAILED(dxCommon_->GetDevice()->CreateGraphicsPipelineState(
                &desc,
                IID_PPV_ARGS(
                    &state_->pipelineStates
                         [target]
                         [static_cast<uint32_t>(PipelineKind::Modulate)])))) {
            resetPipelines();
            return;
        }
