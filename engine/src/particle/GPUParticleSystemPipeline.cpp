...
void GPUParticleSystem::CreateRootSignatures() {
    ...
    {
        static_assert((sizeof(EmitterForGPU) / sizeof(uint32_t)) + 2u + 7u <=
                          64u,
        ...
        if (!RootSignatureUtils::CreateRootSignature(
                dxCommon_->GetDevice(), desc,
                resources_->updateRootSignature)) {
            return;
        }
    }

    resources_->drawRootSignature =
...
void GPUParticleSystem::CreatePipelineStates() {
    ...
    if (FAILED(device->CreateComputePipelineState(
            &computePso, IID_PPV_ARGS(&resources_->updatePso))) ||
    ...
    resources_->drawPso = GpuParticleShared::GetOrCreateDrawPipeline(device, resources_->drawRootSignature.Get(),
                                          pixelShaderPath,
