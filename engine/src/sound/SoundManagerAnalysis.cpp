...
float SoundManager::GetAmplitudeAt(uint32_t soundId, float playbackSeconds,
    ...
    WAVEFORMATEX format{};
    if (!sound.CopyFormat(format) ||
        !SoundFormatUtils::IsSupportedPcmReadFormat(format) ||
    ...
    const float safeWindowSeconds =
        Numeric::ClampFinite(windowSeconds, 0.005f, maxWindowSeconds,
...
void SoundManager::FillSpectrumBands(uint32_t soundId, float playbackSeconds,
    ...
    for (size_t band = 0; band < bandCount; ++band) {
        ...
        const float omega = MathUtils::kTwoPi * frequency / sampleRate;
        double real = 0.0;
        double imag = 0.0;

        for (size_t i = 0; i < kWindowFrames; ++i) {
            const size_t frame =
                (centerFrame + (i % frameCount) + frameCount -
                 ((kWindowFrames / 2) % frameCount)) %
                frameCount;
            const float window =
                0.5f - 0.5f *
                           std::cos(MathUtils::kTwoPi *
                                    static_cast<float>(i) /
