...
void MeshRenderer::DrawMeshShadow(
    ...
    if (!state_->dxCommon || !state_->textureManager ||
        !state_->shadowRootSignature || !state_->shadowPSO ||
    ...
    if (!WriteShadowTransformedDrawConstants(
            drawMaterial, transform, lightViewProjection, objectCbAddr,
    ...
    SubmitShadowMeshDraw(mesh, drawMaterial, objectCbAddr, sceneCbAddr,
...
bool MeshRenderer::WriteShadowIdentityDrawConstants(
    const Material &drawMaterial,
    const DirectX::XMFLOAT4X4 &lightViewProjection,
    D3D12_GPU_VIRTUAL_ADDRESS &objectCbAddr,
    D3D12_GPU_VIRTUAL_ADDRESS &sceneCbAddr,
...
void MeshRenderer::DrawMeshInstancedShadow(
    ...
    uint32_t textureId) {
    if (!state_->dxCommon || !state_->textureManager ||
        !state_->shadowRootSignature || !state_->instancedShadowPSO ||
    ...
    DrawShadowInstancedWithPreparedBuffer(mesh, drawMaterial, instanceView,
                                          instanceCount, lightViewProjection,
                                          nullptr, textureId);
}
...
void MeshRenderer::DrawMeshInstancedShadowWithPipeline(
    ...
    DrawShadowInstancedWithPreparedBuffer(mesh, drawMaterial, instanceView,
                                          instanceCount, lightViewProjection,
                                          &shadowPipelineStates, textureId);
}
...
bool MeshRenderer::DrawInstancedShadowWithPreparedBuffer(
    ...
    const auto &shadowPipelineStates =
        opaqueShadow ? pipelineSet.opaqueShadowPipelineStates
                     : pipelineSet.shadowPipelineStates;
    return DrawShadowInstancedWithPreparedBuffer(
        mesh, drawMaterial, instanceView, instanceCount, lightViewProjection,
        &shadowPipelineStates, textureId);
}
...
bool MeshRenderer::DrawShadowInstancedWithPreparedBuffer(
    ...
    if (!state_->dxCommon || !state_->textureManager ||
        !state_->shadowRootSignature || !IsDrawableMesh(mesh) ||
        instanceCount == 0 || instanceView.BufferLocation == 0 ||
        state_->drawIndex >= kMaxDraws) {
        return false;
    }
    ...
    D3D12_GPU_VIRTUAL_ADDRESS sceneCbAddr = 0;
    D3D12_GPU_VIRTUAL_ADDRESS materialCbAddr = 0;
    if (!WriteShadowIdentityDrawConstants(drawMaterial, lightViewProjection,
                                          objectCbAddr, sceneCbAddr,
                                          materialCbAddr)) {
        return false;
    }
    ...
    if (pipelineStates) {
        ...
        if (!SetInstancedShadowPipelineForMaterial(*pipelineStates,
                                                   drawMaterial)) {
            return false;
        }
    } else {
