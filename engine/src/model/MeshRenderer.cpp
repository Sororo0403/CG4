...
void MeshRenderer::DrawMeshWithPipeline(
    ...
    DrawForwardMeshWithPipelineStates(
        mesh, material, transform, camera,
        &state_->customPipelines[pipelineId].pipelineStates, textureId,
        normalTextureId);
}
...
void MeshRenderer::DrawMeshWithPipelineHandles(
    ...
    DrawForwardMeshWithPipelineHandles(
        mesh, material, transform, camera,
        state_->customPipelines[pipelineId].pipelineStates, textureHandle,
        normalTextureHandle);
}

bool MeshRenderer::DrawForwardMeshWithPipelineStates(
    const Mesh &mesh, const Material &material, const Transform &transform,
    const Camera &camera, const PipelineStateArray *pipelineStates,
    uint32_t textureId, uint32_t normalTextureId) {
    ...
    if (!PrepareForwardMeshDrawConstants(mesh, material, transform, camera,
                                         drawMaterial, objectCbAddr,
    ...
    const bool pipelineReady =
        pipelineStates ? SetPipelineForMaterial(*pipelineStates, drawMaterial)
                       : SetPipelineForMaterial(drawMaterial);
    if (!pipelineReady) {
        return false;
    }
    SubmitForwardMeshDraw(mesh, drawMaterial, objectCbAddr, sceneCbAddr,
                          materialCbAddr, &mesh.vbView, 1, 1, textureId,
                          normalTextureId);
    return true;
}
...
bool MeshRenderer::DrawForwardMeshWithPipelineHandles(
    ...
    SubmitForwardMeshDrawWithHandles(mesh, objectCbAddr, sceneCbAddr,
                                     materialCbAddr, &mesh.vbView, 1, 1,
                                     textureHandle, normalTextureHandle);
    return true;
}

bool MeshRenderer::PrepareForwardMeshDrawConstants(
    const Mesh &mesh, const Material &material, const Transform &transform,
    const Camera &camera, Material &drawMaterial,
    ...
    return WriteForwardTransformedDrawConstants(
        drawMaterial, transform, camera, objectCbAddr, sceneCbAddr,
        materialCbAddr);
}

bool MeshRenderer::WriteForwardTransformedDrawConstants(
...
bool MeshRenderer::WriteForwardIdentityDrawConstants(
    const Material &drawMaterial, const Camera &camera,
    D3D12_GPU_VIRTUAL_ADDRESS &objectCbAddr,
    D3D12_GPU_VIRTUAL_ADDRESS &sceneCbAddr,
    D3D12_GPU_VIRTUAL_ADDRESS &materialCbAddr) {
    objectCbAddr = WriteObjectConstants(XMMatrixIdentity(),
                                        XMMatrixIdentity(),
                                        XMMatrixIdentity());
    sceneCbAddr = WriteSceneConstants(camera);
    materialCbAddr = WriteMaterialConstants(drawMaterial);
...
void MeshRenderer::DrawMeshInstanced(const Mesh &mesh, const Material &material,
    ...
    DrawForwardInstancedWithPreparedBuffer(mesh, drawMaterial, instanceView,
                                           instanceCount, camera, nullptr,
...
bool MeshRenderer::DrawInstancedWithPreparedBuffer(
    ...
    return DrawForwardInstancedWithPreparedBuffer(
        mesh, drawMaterial, instanceView, instanceCount, camera,
        &state_->customInstancedPipelines[pipelineId].pipelineStates, textureId,
        normalTextureId);
}
...
bool MeshRenderer::DrawForwardInstancedWithPreparedBuffer(
    ...
    if (!state_->dxCommon || !state_->textureManager || !state_->rootSignature ||
        ...
        instanceView.BufferLocation == 0 || state_->drawIndex >= kMaxDraws) {
        return false;
    }
    ...
    D3D12_GPU_VIRTUAL_ADDRESS sceneCbAddr = 0;
    D3D12_GPU_VIRTUAL_ADDRESS materialCbAddr = 0;
    if (!WriteForwardIdentityDrawConstants(drawMaterial, camera, objectCbAddr,
                                           sceneCbAddr, materialCbAddr)) {
        return false;
    }
    ...
    SubmitForwardMeshDraw(mesh, drawMaterial, objectCbAddr, sceneCbAddr,
                          materialCbAddr, views, 2, instanceCount, textureId,
                          normalTextureId);
    return true;
}
...
void MeshRenderer::SetSpotLightShadowMap(
    D3D12_GPU_DESCRIPTOR_HANDLE shadowMap,
    const DirectX::XMFLOAT4X4 &lightViewProjection,
    const SceneShadowSettings &settings) {
    RendererShadowMapUtils::Set(
...
void MeshRenderer::SetOcclusionPyramid(
    ...
    XMStoreFloat4x4(&state_->occlusionViewProjection,
                    XMMatrixTranspose(viewProjection));
    state_->occlusionParams = {
