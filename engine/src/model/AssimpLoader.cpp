...
bool AddMeshToInputTotals(const aiMesh &mesh, std::size_t &totalVertices,
                          std::size_t &totalFaces,
                          std::size_t &totalBones) {
    return AddWithinLimit(totalVertices, mesh.mNumVertices,
...
bool AreMeshesWithinInputBudget(const aiScene &scene) {
    ...
    for (unsigned int meshIndex = 0; meshIndex < scene.mNumMeshes;
        ...
        if (!IsMeshWithinInputBudget(*mesh) ||
            !AddMeshToInputTotals(*mesh, totalVertices, totalFaces,
                                  totalBones)) {
            return false;
        }
    }
...
bool AreAnimationsWithinInputBudget(const aiScene &scene) {
    ...
    for (unsigned int animationIndex = 0;
        ...
        if (!AddAnimationChannelsToInputTotals(
                *animation, totalAnimationChannels, totalAnimationKeys)) {
            return false;
        }
    }
    return true;
...
bool IsSceneWithinInputBudget(const aiScene *scene) {
    return HasValidSceneHeader(scene) && HasValidSceneArrays(scene) &&
           AreMeshesWithinInputBudget(*scene) &&
           AreAnimationsWithinInputBudget(*scene);
}
