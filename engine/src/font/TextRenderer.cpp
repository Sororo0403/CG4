...
void AppendCodepoint(std::vector<char32_t> &codepoints, uint32_t value) {
...
std::vector<char32_t> DecodeUtf8(std::string_view text) {
    ...
    while (index < text.size()) {
        ...
        AppendCodepoint(codepoints, value);
        index += length;
    }
        ...
        if constexpr (sizeof(wchar_t) == 2u) {
            if (value >= 0xD800u && value <= 0xDBFFu) {
                if (index + 1u < text.size()) {
                    const uint32_t low =
                        static_cast<uint32_t>(text[index + 1u]);
...
TextLayoutMetrics MeasureCodepoints(FontManager &fontManager,
    ...
    const float lineAdvance =
        fontMetrics.lineHeight + ResolveLineSpacing(style.lineSpacing);
    ...
    for (char32_t codepoint : codepoints) {
        ...
        const FontGlyph *glyph =
            fontManager.GetGlyph(font, style.pixelSize, codepoint);
        if (glyph != nullptr) {
...
void DrawCodepoints(FontManager &fontManager, SpriteRenderer &spriteRenderer,
    ...
    for (char32_t codepoint : codepoints) {
        ...
        if (glyph->visible && IsValidResourceId(glyph->textureId)) {
            Sprite sprite{};
            sprite.position = {cursorX + glyph->offset.x,
                               baselineY + glyph->offset.y};
...
void TextRenderer::Initialize(FontManager *fontManager,
...
bool TextRenderer::IsReady() const {
    return fontManager_ != nullptr && spriteRenderer_ != nullptr &&
           fontManager_->IsReady();
}

void TextRenderer::DrawText(std::string_view utf8Text,
                            const DirectX::XMFLOAT2 &position,
                            const TextStyle &style) {
    if (!IsReady()) {
        return;
    }
    DrawCodepoints(*fontManager_, *spriteRenderer_, DecodeUtf8(utf8Text),
                   position, style);
}

void TextRenderer::DrawText(std::wstring_view text,
                            const DirectX::XMFLOAT2 &position,
                            const TextStyle &style) {
    if (!IsReady()) {
        return;
    }
    DrawCodepoints(*fontManager_, *spriteRenderer_, DecodeWide(text), position,
                   style);
}

void TextRenderer::DrawText(std::u32string_view text,
                            const DirectX::XMFLOAT2 &position,
                            const TextStyle &style) {
    if (!IsReady()) {
        return;
    }
    DrawCodepoints(*fontManager_, *spriteRenderer_, CopyCodepoints(text),
                   position, style);
}

TextLayoutMetrics TextRenderer::MeasureText(std::string_view utf8Text,
                                            const TextStyle &style) {
    if (!IsReady()) {
        return {};
    }
...
TextLayoutMetrics TextRenderer::MeasureText(std::wstring_view text,
                                            const TextStyle &style) {
    if (!IsReady()) {
        return {};
    }
...
TextLayoutMetrics TextRenderer::MeasureText(std::u32string_view text,
                                            const TextStyle &style) {
    if (!IsReady()) {
        return {};
    }
