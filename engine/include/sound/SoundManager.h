...
class SoundManager : public ISoundService {
  public:
    ...
    SoundHandle CreatePcm16SoundHandle(
        ...
        const std::vector<int16_t> &pcmSamples, uint32_t sampleRate = 48000,
        uint16_t channels = 1) {
        return SoundHandle(
            CreatePcm16Sound(cacheKey, pcmSamples, sampleRate, channels));
    }
    ...
    uint32_t PlayStream(const std::wstring &path, float volume = 1.0f,
                        bool loop = false);
    VoiceHandle PlayStreamHandle(const std::wstring &path,
                                 float volume = 1.0f, bool loop = false) {
    ...
    void Pause(VoiceHandle voice) { Pause(voice.Get()); }

    /// <summary>
    /// 一時停止したボイスを再開する
    /// </summary>
    void Resume(uint32_t voiceHandle);
    void Resume(VoiceHandle voice) { Resume(voice.Get()); }
    ...
    void FillSpectrumBands(SoundHandle sound, float playbackSeconds,
                           float *outBands, size_t bandCount) const {
