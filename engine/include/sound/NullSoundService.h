...
class NullSoundService final : public ISoundService {
  public:
    uint32_t CreatePcm16Sound(const std::wstring &,
                              const std::vector<int16_t> &, uint32_t = 48000,
                              uint16_t = 1) override {
    ...
    uint32_t PlayFrom(uint32_t, float, float = 1.0f,
                      bool = false) override {
        return kInvalidVoiceHandle;
    }

    uint32_t Play3D(uint32_t, const DirectX::XMFLOAT3 &, float = 1.0f,
                    bool = false) override {
        return kInvalidVoiceHandle;
    }
    ...
    bool IsPlaying(uint32_t) const override { return false; }
    const SoundInfo *GetInfo(uint32_t) const override { return nullptr; }
