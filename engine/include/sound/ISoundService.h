...
class ISoundService {
  public:
    ...
    virtual uint32_t Play(uint32_t soundId, float volume = 1.0f,
                          bool loop = false) = 0;
    virtual uint32_t PlayFrom(uint32_t soundId, float startSeconds,
                              float volume = 1.0f, bool loop = false) = 0;
    virtual uint32_t Play3D(uint32_t soundId,
    ...
    virtual void SetVoiceFrequencyRatio(uint32_t voiceHandle,
                                        float frequencyRatio) = 0;
    virtual void SetVoicePosition(
        uint32_t voiceHandle,
        const DirectX::XMFLOAT3 &sourcePosition) = 0;
    virtual void SetVoice3DRange(uint32_t voiceHandle, float minDistance,
                                 float maxDistance) = 0;
