...
inline void AddTriangleIndices(Geometry &geometry, uint32_t a, uint32_t b,
                               uint32_t c) {
    geometry.indices.insert(geometry.indices.end(), {a, b, c});
}

template <typename Geometry>
inline void AddQuadIndices(Geometry &geometry, uint32_t a, uint32_t b,
                           uint32_t c, uint32_t d) {
    geometry.indices.insert(geometry.indices.end(), {a, b, c, a, c, d});
}

template <typename Geometry>
inline void AddReversedQuadIndices(Geometry &geometry, uint32_t a,
