...
inline bool IsMetadataWithinBudget(const DirectX::TexMetadata &metadata,
    ...
    if (metadata.width > kMaxDimension || metadata.height > kMaxDimension ||
        metadata.arraySize > kMaxArraySize ||
        metadata.mipLevels > kMaxMipLevels) {
        return false;
    }
    ...
    if (metadata.arraySize >
        (std::numeric_limits<std::size_t>::max)() / metadata.mipLevels) {
        return false;
    }
    const std::size_t expectedImageCount =
        static_cast<std::size_t>(metadata.arraySize) *
        static_cast<std::size_t>(metadata.mipLevels);
...
inline bool AreImagesWithinDecodedBudget(const DirectX::Image *images,
    ...
    for (std::size_t index = 0; index < imageCount; ++index) {
        ...
        if (image.pixels == nullptr || image.rowPitch == 0 ||
            image.slicePitch == 0) {
            return false;
        }
